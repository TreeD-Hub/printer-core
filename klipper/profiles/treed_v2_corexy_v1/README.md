...
без слепого хода. Геометрия ниже измерена; параметры StallGuard ещё не приняты аппаратно.
...
| `bottom_position` | 203 мм | Измеренная координата нижнего контакта; не выше position_max. |
| `max_seek` | 208 мм | Жёсткий предел поиска; не больше bottom_position − position_min. |
