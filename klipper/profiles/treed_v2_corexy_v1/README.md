...
- при неизвестной Z перед X/Y выполняет `FORCE_MOVE` оси Z на `z_hop` вниз от сопла; только полный `G28` временно отмечает Z как привязанную до Eddy Z-home, а отдельный `G28 X/Y` оставляет Z неизвестной;
...
- отдельный `G28 Z` требует готовые X/Y; внутри Eddy-контура неизвестная Z не блокирует поиск датчиком.

После перезапуска, отключения моторов или прерванного homing полный `G28` восстанавливает координаты без отдельного макроса. `FORCE_MOVE` не проверяет предел Z: если стол уже у нижнего механического упора, привод может упереться в него. Временная Z-позиция служит только для прохода к Eddy; достоверная Z появляется после завершения полного `G28`.
