...
После перезапуска или отключения моторов полный `G28` использует DIAG для неизвестной Z. При уже известной Z поиск пропускается. После `G28 X/Y` Z остаётся временной; рабочий Z0, измеренный доступный ход и первый слой требуют полного `G28` или последующего `G28 Z` через Eddy.
...
В режиме `motion` отсутствие движения при сработавшем switch считается возможным засором. Принтер сохраняет рабочий нагрев, повышает цель хотэнда и ток экструдера не более чем на 20% в заданных безопасных пределах и делает до пяти подач по `25` мм со скоростью `5` мм/с. При восстановлении движения печать продолжается; иначе ток возвращается, принтер остаётся на паузе, а хотэнд переводится на `140` °C.
...
| `bottom_clearance_mm` | 5 мм | Отход от DIAG и запас рабочего лимита Z относительно измеренной высоты контакта. |
...
привязывается к 198 мм с таким же временным верхним лимитом. Рабочий Z0 затем
определяет Eddy. При отказе Z сбрасывается в unknown. Исходные поля TMC
(включая ток и SGT) и лимиты скорости/ускорения возвращаются
...
`TREED_Z_PARK_BOTTOM_MANUAL` принудительно повторяет тот же поиск в фазе `idle`,
но после безопасного отхода не запускает автосъём и не отключает моторы.

`TREED_Z_PARK_BOTTOM` использует тот же DIAG-контур, но разрешён только в фазе
`auto_remove` и принудительно ищет нижнюю опору даже при известной Z. После
безопасного отхода выполняются пять циклов `bottom_safe -> bottom_safe - 25 ->
bottom_safe` на 50 мм/с. Цикл начинается только после подтверждённого DIAG и
проверки 25 мм допустимого хода; любой отказ останавливает последовательность.
Глобальный Z-лимит должен допускать 50 мм/с, отдельного обхода лимитов нет.

Для определения доступного хода extra сохраняет MCU-позицию первого DIAG,
исключая выбег после срабатывания. После `G28.1 Z`, точного `PROBE` и коррекции Z0
команда `_TREED_Z_TRAVEL_APPLY` вычисляет высоту контакта относительно Eddy:
`contact_z = contact_mcu_mm - (current_mcu_mm - corrected_z)`.
Рабочий максимум равен `min(stepper_z.position_max, contact_z - bottom_clearance_mm)`.
Например, контакт на Z=180 даёт максимум 175 мм при запасе 5 мм.
Лимит применяется до финального Z-hop: в CoreXY check_move, диапазоне рельса,
`toolhead.axis_maximum.z` и runtime-поле `_TREED_UI_CONTRACT.axis_z_max`.
Повторный Z-home пересчитывает лимит от той же MCU-опоры; смена координат
не возвращает прежние 203 мм. Слайсер и файлы конфигурации автоматически не меняются.
Обычная команда движения выше измеренного максимума завершается ошибкой диапазона.

Текущее измерение доступно в `printer.treed_z_recovery.z_travel`:
`state=pending_eddy` до Eddy, `state=measured`, `contact_z` и `z_max` после него.
Отключение мотора, shutdown или ошибка Eddy сбрасывают измерение и homed Z;
для восстановления нужен полный `G28`. Потеря общей MCU-опоры, изменение шага
или некорректный/недостаточный ход дают ошибку без продолжения с фиксированными 203 мм.
Расстояние по шагам описывает найденный контакт, не доказывает отсутствие
препятствий по всей области XY и требует отсутствия потерянных шагов до Eddy.

Печать, пауза, фаза калибровки, активный manual probe и SGT-калибровка запрещают
обычный recovery. Фаза `preparing` разрешена внутри `START_PRINT`, даже когда
virtual_sd уже имеет статус printing. Фаза `auto_remove` разрешена только для
`TREED_Z_PARK_BOTTOM`; после перехода в `printing` обычный recovery закрыт.
