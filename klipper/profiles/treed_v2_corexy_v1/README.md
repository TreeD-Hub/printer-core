...
- при неизвестной Z вызывает `TREED_Z_HOME_BOTTOM`: поиск нижнего упора, отход, повторная проверка расстояния и финальный отход; ошибка останавливает G28 до X/Y;
...
По умолчанию **`enabled: False`**: неизвестная Z останавливает homing с ошибкой,
без слепого хода. Начальные значения ниже — кандидаты, а не проверенная настройка.
Оператор проверяет направление `+Z = стол вниз`, подключение DIAG к PG10,
свободный ход до нижнего упора и пригодность механики для контакта. Затем
вносит параметры в runtime `local_overrides.cfg` и явно включает контур
`[treed_z_recovery]` с `enabled: True` для контролируемых проб. Этот файл сохраняется
в режиме loader `preserve`; автоматического сохранения результатов нет.
...
| `speed` | 5 мм/с | Одинаковая скорость обоих поисков и отходов. |
| `accel` | 100 мм/с² | Временный потолок ускорения, не повышает рабочие лимиты. |
| `bottom_position` | 255 мм | Измеренная координата нижнего контакта; не выше position_max. |
| `max_seek` | 260 мм | Жёсткий предел первого поиска; не больше bottom_position − position_min. |
| `verify_backoff_mm` | 5 мм | Отход после первого контакта к соплу. |
| `tolerance` | 0.5 мм | Допуск хода второй пробы, меньше половины отхода. |
| `bottom_clearance_mm` | 5 мм | Финальная координата bottom_position − clearance. |
| `stallguard_pause` | 2 с | Покой перед каждой пробой, минимум 2 с. |

`TREED_Z_HOME_BOTTOM` не принимает G-code параметров и пропускает поиск при
известной Z. Перед каждой пробой завершается очередь и выдерживается пауза.
Поиск идёт через `HomingMove`; второй ограничен `verify_backoff_mm + tolerance`.
Измеряется MCU trigger position, а не заданная конечная точка. Внутри homing
используется временная система координат, как в Klipper; при отказе Z сбрасывается
