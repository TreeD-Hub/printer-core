...
- при неизвестной Z перед X/Y выполняет `FORCE_MOVE` оси Z на `z_hop` вниз от сопла, временно отмечает только Z как привязанную, затем выполняет sensorless X/Y и Eddy Z-home;
