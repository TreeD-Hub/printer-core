...
- при неизвестной Z все варианты `G28` завершаются до движения: нужно независимо определить положение стола и восстановить достоверную Z-позицию;
...
- отдельный `G28 Z` требует уже homed X/Y и известную Z, иначе будет явная ошибка.

Практический порядок после подтверждения Z-позиции:

```gcode
G28
```

Отдельный `G28 Z` использовать только после успешного `G28 X Y` или полного `G28`. После сбоя автосъём не должен пытаться автоматически выполнить `G28` при неизвестной Z.
...
Первичная калибровка Eddy с учетом безопасной scan area и runtime `[force_move]`. Макрос требует предварительно достоверной Z-позиции: при неизвестной Z его `G28 X Y` блокируется до движения. После проверки Z ставит Eddy в центр `X10..235 / Y10..210` и запускает штатный `PROBE_EDDY_CURRENT_CALIBRATE`.
