...
- при неизвестной Z перед X/Y выполняется `FORCE_MOVE` на `z_hop` от сопла; позиция Z временно задаётся для XY-переезда и заменяется результатом Eddy Z-home при полном `G28`;
