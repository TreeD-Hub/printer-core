...
- при неизвестной Z перед X/Y выполняется `FORCE_MOVE` на `z_hop` от сопла; только полный `G28` временно задаёт Z до Eddy Z-home, а отдельный `G28 X/Y` оставляет её неизвестной;
...
- отдельный `G28 Z` требует уже homed X/Y; неизвестная Z не блокирует поиск датчиком.
...
После успешного `G28` для парковки можно выполнить `G1 X10 Y235 F6000` в сервисных raw-координатах. Отдельный `G28 Z` использовать только после успешного `G28 X Y` или полного `G28`. При неизвестной Z и X/Y команда `G28 Z` не заменяет полный homing. `FORCE_MOVE` не проверяет нижний предел стола; если стол уже у упора, привод может в него упереться.
