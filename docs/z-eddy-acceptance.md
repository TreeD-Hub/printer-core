...
после него временная Z и верхний лимит равны 198 мм до Eddy Z0.
После Eddy рабочий максимум равен `min(203, измеренная высота DIAG - 5)`;
параметры берутся из runtime config. Измерение хранится только до потери опоры:
отключения Z, shutdown или ошибки Eddy. Слайсер и рабочие токи не калибруются.
...
После `final_z0` проверить `treed_z_recovery.z_travel`: `state=measured`,
`contact_z` совпадает с независимо измеренным расстоянием до контакта,
`z_max` учитывает запас. `toolhead.axis_maximum.z` должен совпадать с `z_max`.
Отдельная bottom-серия оставляет `state=pending_eddy` и не измеряет рабочую высоту.
Офлайн-тест включает ранний контакт на 180 мм и отказ движения выше 175 мм;
это проверка расчёта и закреплённого CoreXY, а не аппаратное доказательство.
