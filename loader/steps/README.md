...
| 16 | `klipper-profiles.sh` | required | Профиль V2: main USB serial, EBB CAN UUID, Eddy UUID, контур X/Y sensorless (tmc2209 virtual endstop) и Z на tmc5160 SPI. |
...
- Для `treed_v2_corexy_v1` X/Y homing работает в sensorless-контуре (`tmc2209_stepper_x/y:virtual_endstop`): перед deploy требуются TMC2209 в UART-режиме, DIAG-джамперы на X/Y и отключение X/Y механических endstop из логики. Z работает на TMC5160 SPI в слоте `MOTOR2_1`.
