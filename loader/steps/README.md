...
- Для `treed_v2_corexy_v1` X/Y homing работает в sensorless-контуре (`tmc5160_stepper_x/y:virtual_endstop`): перед deploy требуются TMC5160/TMC5160T Pro, корректная SPI/DIAG обвязка на X/Y и отключение X/Y механических endstop из логики. При Eddy enabled `stepper_z` использует `probe:z_virtual_endstop`, штатный `G28` не переопределяется, а `TREED_Z_PARK_ZERO_EDDY` уточняет рабочий Z0 через `PROBE` после сохраненной Eddy-калибровки; Zmax DIAG на `PG10` остается аппаратным резервом вне штатного G28-контура.
