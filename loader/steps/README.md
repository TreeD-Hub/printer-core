...
- Для `treed_v2_corexy_v1` X/Y homing работает через `tmc5160_stepper_x/y:virtual_endstop`. Штатный `G28` в `macros_homing.cfg` вызывает `G28.1`, делает отход от X-min/Y-max на 10 мм и ждёт не менее 2 секунд перед каждым sensorless-проходом. Отдельный сервисный инструмент `treed-sensorless-calibrate` и Klipper-компонент `treed_motor_sensorless.py` устанавливаются `runtime-bootstrap.sh`; их использование описано в `docs/sensorless-calibration.md`.
