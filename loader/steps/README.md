...
- Для `treed_v2_corexy_v1` X/Y homing работает в sensorless-контуре (`tmc5160_stepper_x/y:virtual_endstop`): перед deploy требуются TMC5160/TMC5160T Pro, корректная SPI/DIAG обвязка на X/Y и отключение X/Y механических endstop из логики. При Eddy enabled `stepper_z` использует `probe:z_virtual_endstop`, а `G28 Z`/полный `G28`/`TREED_Z_PARK_ZERO_EDDY` ищут рабочий Z0 через Eddy после сохраненной Eddy-калибровки; Zmax DIAG на `PG10` остается аппаратным резервом вне штатного G28-контура.
