...
TREED_CAN_TXQUEUE="${TREED_CAN_TXQUEUE:-128}"
...
case "${TREED_KLIPPER_PREFLIGHT}" in
  0|1) ;;
  *)
    log_error "check-env: TREED_KLIPPER_PREFLIGHT must be 0 or 1, got: ${TREED_KLIPPER_PREFLIGHT}"
    exit 1
    ;;
esac
case "${TREED_KLIPPER_PREFLIGHT_CAN_UUIDS_REQUIRED}" in
  0|1) ;;
  *)
    log_error "check-env: TREED_KLIPPER_PREFLIGHT_CAN_UUIDS_REQUIRED must be 0 or 1, got: ${TREED_KLIPPER_PREFLIGHT_CAN_UUIDS_REQUIRED}"
    exit 1
    ;;
esac
case "${TREED_KLIPPER_PREFLIGHT_WAIT_SEC}" in
  ''|*[!0-9]*)
    log_error "check-env: TREED_KLIPPER_PREFLIGHT_WAIT_SEC must be a positive integer, got: ${TREED_KLIPPER_PREFLIGHT_WAIT_SEC}"
    exit 1
    ;;
esac
case "${TREED_KLIPPER_PREFLIGHT_INTERVAL_SEC}" in
  ''|*[!0-9]*)
    log_error "check-env: TREED_KLIPPER_PREFLIGHT_INTERVAL_SEC must be a positive integer, got: ${TREED_KLIPPER_PREFLIGHT_INTERVAL_SEC}"
    exit 1
    ;;
esac
...
if [ "${TREED_KLIPPER_PREFLIGHT_WAIT_SEC}" -le 0 ] || [ "${TREED_KLIPPER_PREFLIGHT_INTERVAL_SEC}" -le 0 ]; then
  log_error "check-env: TREED_KLIPPER_PREFLIGHT_WAIT_SEC and TREED_KLIPPER_PREFLIGHT_INTERVAL_SEC must be > 0"
  exit 1
fi
