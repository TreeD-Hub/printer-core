...
$macrosPurge = Read-RepoFile "klipper/profiles/treed_v2_corexy_v1/macros_start_purge.cfg"
...
$purgeSettings = Get-GcodeMacroBlock $macrosPurge "_TREED_START_PURGE_CFG"
$smartPark = Get-GcodeMacroBlock $macrosPurge "_TREED_SMART_PARK"
$linePurge = Get-GcodeMacroBlock $macrosPurge "_TREED_LINE_PURGE"
...
Assert-Contains $zHopBeforeXy '(?m)^\s*TREED_Z_HOME_BOTTOM\s*$' "Unknown Z must use the bounded sensorless recovery executor"
Assert-NotContains $zHopBeforeXy 'FORCE_MOVE|SET_KINEMATIC_POSITION|MARK_Z' "Unknown Z must not use blind motion or invent a coordinate in a macro"
...
Assert-ContainsBefore $g28 '(?m)^\s*BED_MESH_CLEAR\s*$' '(?m)^\s*_TREED_Z_HOP_BEFORE_XY\s*$' "G28 must clear stale mesh before homing travel"
Assert-ContainsBefore $g28 '(?m)^\s*_TREED_PRINT_OFFSET_DISABLE\s*$' '(?m)^\s*_TREED_Z_HOP_BEFORE_XY\s*$' "G28 must use raw XY coordinates before homing travel"
Assert-ContainsBefore $g28 '(?m)^\s*_TREED_UI_RESET_Z_OFFSET\s*$' '(?m)^\s*_TREED_Z_HOP_BEFORE_XY\s*$' "G28 must clear stale Z offset before homing travel"
...
Assert-Contains $purgeSettings 'variable_park_height:\s*10\.0' "SMART_PARK must wait above the bed for final heating"
...
Assert-Contains $smartPark '(?s)G0 X\{park_x\} Y\{park_y\} F6000\s*\{% if th\.position\.z\|float > park_z %\}\s*G0 Z\{park_z\} F1500' "SMART_PARK must move XY before lowering to heat Z"
...
Assert-ContainsBefore $startPrint '(?m)^\s*_TREED_START_FINAL_HEAT\s*$' '(?m)^\s*_TREED_LINE_PURGE\s*$' "START_PRINT must heat nozzle before LINE_PURGE"
...
Assert-Contains $smartPark '"x" not in homed or "y" not in homed or "z" not in homed' "park requires homed XYZ"
Assert-Contains $linePurge '"x" not in homed or "y" not in homed or "z" not in homed' "purge requires homed XYZ"
