...
Assert-ContainsBefore $startPrint '(?m)^\s*_TREED_HOME_ALL\s*$' '(?m)^\s*_TREED_START_ADAPTIVE_MESH\s*$' "Mesh строится после homing"
Assert-ContainsBefore $startPrint '(?m)^\s*_TREED_START_ADAPTIVE_MESH\s*$' '(?m)^\s*_TREED_SMART_PARK\s*$' "Smart park выполняется после mesh"
...
Assert-ContainsBefore $startPrint '(?m)^\s*_TREED_PRINT_OFFSET_ENABLE\s*$' '(?m)^\s*_TREED_SMART_PARK\s*$' "Smart park должен работать в print coords"
