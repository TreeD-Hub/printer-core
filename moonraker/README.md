...
- если найден валидный git checkout Crowsnest (по умолчанию `${PI_HOME}/crowsnest`, с `.git` и метаданными updater: legacy `tools/pkglist.sh` или v5 `system-dependencies.json` + `requirements.txt`), updater-секция Crowsnest остается активной;
