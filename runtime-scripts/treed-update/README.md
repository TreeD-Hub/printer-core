...
- `treed-update-apply printer-core vX.Y.Z` — отказывается перезаписывать dirty checkout, переключается на release tag без forced checkout и запускает `install.sh` в `TREED_DEPLOY_MODE=auto`.
